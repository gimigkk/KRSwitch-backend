...
// ─── STUDENTS ─────────────────────────────────────────────────────────────────
const users = [
  { nim: 'M0403241075', name: 'Muh Arifaushan', email: 'muharifaushan@apps.ipb.ac.id', role: 'student' },
  { nim: 'M0403241117', name: 'Gilang Muhamad Widiagung', email: 'gnaligilang@apps.ipb.ac.id', role: 'student' },
  { nim: 'M0403241029', name: 'Azka Julian', email: 'azkajulian2@apps.ipb.ac.id', role: 'student' },
...
function validate() {
  ...
  for (const user of users) {
    ...
    for (const code of COURSE_CODES) {
      const types = courseMap[code].types;
      const pracKey = 'P' in types ? 'P' : 'R' in types ? 'R' : null;
      if (ids.filter(id => classById[id]?.courseCode === code && classById[id]?.classType === 'K').length !== 1) { console.error(`[ENROLL] ${user.nim} missing K for ${code}`); errors++; }
      if (pracKey && ids.filter(id => classById[id]?.courseCode === code && classById[id]?.classType !== 'K').length !== 1) { console.error(`[ENROLL] ${user.nim} missing P/R for ${code}`); errors++; }
    }
...
function addMatchedSwap(courseCode, classType, createdAt, completedAt) {
  ...
  for (const u of users) {
    ...
    for (const id of scheduleByNim[u.nim])
      if (classById[id].courseCode === courseCode && classById[id].classType === classType) { cands.push({ nim: u.nim, cid: id }); break; }
  }
...
async function main() {
  ...
  for (const offer of offerRecords) {
    ...
    notifications.push({
      ...
      data: {
        offerId: inserted.id, takerNim: taker.nim, takerName: taker.name,
    ...
    notifications.push({
      ...
      data: {
        offerId: inserted.id, offererNim: offerer.nim, offererName: offerer.name,
